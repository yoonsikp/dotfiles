...
if [ "$1" = 'toggle' ]; then
  ...
  if [ -n "$state" ]; then
    kill $(ps -e | grep "ssh pi@home.naut.ca -p 8080 -D 8081 -Nf" | grep -v grep | head -n1 | cut -d " " -f2)
    ssh pi@home.naut.ca -p 8080 -D 8081 -Nf
...
echo "$proxy_type $state_icon | dropdown=false"
